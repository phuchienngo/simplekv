...
public class SwissMap<K, V> extends AbstractMap<K, V> {
  ...
  private void add(K k, V v, int hi, byte lo) {
    final var g = this.group;
    var i = g.mod(hi);
    final var p = this.platform;
    var s = p.shift();
    var m = g.meta;
    var d = m.data;
    while (true) {
  ...
  public V get(final Object key) {
    ...
    while (true) {
      //
      var x = i << s;
      final var simd = p.simd(x, d);
      var t = p.eq(cast(simd), lo);
      while (t != 0L) {
        var n = zeros(t);
        final var y = p.next(n);
        var j = x + y;
        final var k = g.getKey(j);
        if (!equals(key, k)) {
          t &= ~(1L << n);
        } else {
          final V r = g.getValue(j);
          return r;
        }
      }
    ...
    while (true) {
      ...
      while (t != 0L) {
        ...
        if (!equals(key, k)) {
          ...
          return this.entry(k, g.getValue(j));
        }
      }
  ...
  private class Meta {
    ...
    void clear() {
      fill(this.data, EMPTY);
    }

    void add(int i, byte v) {
      data[i] = v;
    }
  ...
  private class Group {
    ...
    public boolean containsValues(Object o) {
      final var self = SwissMap.this;
      final var v = this.vals;
      final var d = meta.data;
      for (int i = 0, n = d.length; i < n; i++) {
        if (Meta.exists(d, i)) {
          if (self.equals(o, v[i])) return true;
        }
      }
      return false;
    }
  ...
  abstract class AbstractIterator<T> implements Iterator<T> {
    ...
    int index;
    Group group = SwissMap.this.group;

    @Override
    public boolean hasNext() {
      var r = false;
      final var d = group.meta.data;
      int i = index, n = d.length;
      for (; i < n; i++) {
        if (Meta.exists(d, i)) {
          r = true;
          break;
  ...
  class XEntryIterator extends AbstractIterator<Entry<K, V>> {
    ...
    public Entry<K, V> next() {
      ...
      index = i + 1;
      var key = g.getKey(i);
      var value = g.getValue(i);
      var r = next = new XEntry(key, value, i);
      return r;
    }
    ...
    public void remove() {
      ...
      SwissMap.this.remove(key, value);
      this.next = (null);
    }
  }
  ...
  private class XEntry extends AbstractMap.SimpleEntry<K, V> {
    ...
    public V setValue(final V v) {
      V r = super.setValue(v);
      var g = group;
      var i = index;
      if (Meta.exists(g.meta.data, i)) {
        g.vals[i] = (v);
      }
      return r;
    }
  ...
  private class XKeySet extends AbstractSet<K> {
    ...
    @Override
    public int size() {
      return SwissMap.this.size();
    }

    @Override
    public Object[] toArray() {
      return group.toKeyArray();
    }

    @Override
    public boolean isEmpty() {
      return SwissMap.this.isEmpty();
    }

    @Override
    public Iterator<K> iterator() {
      return new XKeyIterator();
    }

    @Override
    public <T> T[] toArray(T[] o) {
      return group.toKeyArray(o);
    }

    @Override
    public boolean remove(Object k) {
      return SwissMap.this.delete(k);
    }

    @Override
    public boolean contains(Object k) {
      return SwissMap.this.containsKey(k);
    }
  }
  ...
  private class XValues extends AbstractCollection<V> {
    ...
    public Iterator<V> iterator() {
      return new XValueIterator();
    }

    @Override
    public <T> T[] toArray(T[] o) {
      return group.toValueArray(o);
    }

    @Override
    public boolean contains(Object v) {
      return SwissMap.this.containsValue(v);
    }
  }
  ...
  private class XEntrySet extends AbstractSet<Entry<K, V>> {
    ...
    public boolean remove(Object o) {
      ...
      var e = (Entry<K, V>) o;
      final var r = SwissMap.this.remove(e.getKey(), e.getValue());
      return r;
    }
    ...
    public boolean contains(Object o) {
      ...
      var n = node(k);
      var r = (n != null && SwissMap.this.equals(n.getValue(), v));
      return r;
    }
  }
  ...
  public static class Platform64Ex implements Platform<Long> {
    ...
    @Override
    public int maxAvgGroupLoad() {
      return 7;
    }

    @Override
    public Long simd(final int i, byte[] a) {
      return UNSAFE.getLong(a, OFFSET + i); /* native */
    }

    @Override
    public long empty(final Long v) {
      long r = v.longValue() ^ (HI);
      return (r - LO) & ~r & HI;
    }

    @Override
    public long eq(final Long v, final byte w) {
      long r = v.longValue() ^ (LO * w);
      return (r - LO) & ~r & HI;
    }
  }
}